...
class TestEndToEndWorkflow:
    ...
    async def test_complete_audit_search_history_flow(
        ...
        async with AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as client:
            ...
            uploaded_scan_key = [k for k in scan_keys if audit_id in k and "e2e_test.tf" in k]
            ...
            patched_keys = [item["key"] for item in storage.list_files(prefix="patches")]
            uploaded_patched_key = [k for k in patched_keys if audit_id in k and "e2e_test.tf" in k]
            ...
            # Semantic search
            search_payload = {
                "query": "Port 22 is open to the internet",
                "limit": 10
            }
            ...
            our_results = [r for r in search_data["results"] if r["audit_id"] == audit_id]
            ...
            s3_client.delete_object(Bucket=settings.s3_bucket_name, Key=uploaded_scan_key[0])
            s3_client.delete_object(Bucket=settings.s3_bucket_name, Key=uploaded_patched_key[0])
